...
def get_event_logs_statistics(event_logs, use_categorical_attributes):
	...
	for event_log in event_logs:
		...
		per_event_log_statistics.append(list(statistics[event_log].values()) + [event_log])
	...
	statistics = pd.DataFrame(columns=statistics_columns + ["label"], data = per_event_log_statistics)
...
if mode == "training":
	...
	event_logs_statistics = get_event_logs_statistics(event_logs, use_categorical_attributes)
	event_logs_statistics = event_logs_statistics.drop(labels="label", axis=1)
	
	train_event_logs_statistics, validation_event_logs_statistics = train_test_split(event_logs_statistics, test_size = validation_split_percentage)
	
	# The following piece of code should be replaced to address the case where features from the training and validation sets differ. For now, this works, but real implementations need to address this.
	event_logs_statistics = event_logs_statistics.reindex(sorted(event_logs_statistics.columns), axis=1)
	
	train_event_logs_statistics, normalization_parameters = normalize_dataset(train_event_logs_statistics, 0, None)
	train_event_logs_statistics, compression_parameters = compress_dataset(train_event_logs_statistics, n_pca_components, 0, None)
	validation_event_logs_statistics, ignore = normalize_dataset(validation_event_logs_statistics, 1, normalization_parameters)
	validation_event_logs_statistics, ignore = compress_dataset(validation_event_logs_statistics, n_pca_components, 1, compression_parameters)
	model, threshold = train_knn(train_event_logs_statistics, validation_event_logs_statistics, n_neighbors)
	...
	normal_labels = read_normal_labels(mode, run_mode)
	normal_event_logs = get_normal_event_logs(event_logs, normal_labels)
	normal_event_logs_statistics = get_event_logs_statistics(normal_event_logs, use_categorical_attributes)
	normal_event_logs_statistics = normal_event_logs_statistics.drop(labels="label", axis=1)
	normal_features = list(normal_event_logs_statistics.columns)
	if sample_time == 1:
		tm_1 = 0
		tm_1 = time.time()
	all_event_logs_statistics = get_event_logs_statistics(event_logs, use_categorical_attributes)
	if sample_time == 1:
		tm_1 = time.time() - tm_1
	write_features(all_event_logs_statistics, run_mode)	
	all_event_logs_statistics = all_event_logs_statistics.drop(labels="label", axis=1)
	all_features = list(all_event_logs_statistics.columns)
	for feature in all_features:
		if feature not in normal_features:
			all_event_logs_statistics = all_event_logs_statistics.drop(labels=feature, axis=1)
	all_event_logs_statistics = all_event_logs_statistics.reindex(sorted(all_event_logs_statistics.columns), axis=1)		
	all_event_logs_statistics, ignore = normalize_dataset(all_event_logs_statistics, 1, normalization_parameters)
	all_event_logs_statistics, ignore = compress_dataset(all_event_logs_statistics, n_pca_components, 1, compression_parameters)
	if sample_time == 1:
		tm_2 = 0
		tm_2 = time.time()
	classifications = classify_diagnoses(threshold, model, all_event_logs_statistics)
	if sample_time == 1:

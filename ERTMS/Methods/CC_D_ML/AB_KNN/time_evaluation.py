...
def load_times():
	...
	for line in file.readlines():
		ad_times.append(float(line.strip("\n")))
	file.close()
	
	file = open(input_firstrun_dir + "DG_timing_FirstRun.txt")
	for line in file.readlines():
		dg_times.append(float(line.strip("\n")))
	file.close()
	
	
	return dg_times, ad_times
...
dg_times, ad_times = load_times()
times = get_times(dg_times, ad_times)
write_times(times)

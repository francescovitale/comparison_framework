...
for /l %%x in (1, 1, %nreps%) do (
	...
	python anomaly_detection.py FirstRun training %debug_mode% %sample_time% %noise_threshold% %cc_variant% %validation_split_percentage%
		
	copy Output\FirstRun\AD\Training\Model\* Input\FirstRun\AD\Inference\Model

	python anomaly_detection.py FirstRun inference %debug_mode% %sample_time% %noise_threshold% %cc_variant%
		
	ren Output\FirstRun\AD\Training\Model\NormativeModel.pnml NormativeModel_%%x.pnml	
	ren Output\FirstRun\AD\Training\Model\threshold.txt threshold_%%x.txt

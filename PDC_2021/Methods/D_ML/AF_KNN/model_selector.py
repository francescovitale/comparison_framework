...
def clean_environment(best_accuracy_model, run_mode):	
	...
	if run_mode == "FirstRun":
		...
		for normalization_parameters_filename in os.listdir(input_dir + run_mode + "/AD/Inference/Normalization"):
			if normalization_parameters_filename.split(".")[0].split("_")[-1] != str(best_accuracy_model):
				os.remove(input_dir + run_mode + "/AD/Inference/Normalization/" + normalization_parameters_filename)
			else:
				normalization_parameters_filename_tokens = normalization_parameters_filename.split(".")[0].split("_")
				new_normalization_parameters_filename = ""
				for idx,token in enumerate(normalization_parameters_filename_tokens):
					if idx < len(normalization_parameters_filename_tokens)-2:
						new_normalization_parameters_filename = new_normalization_parameters_filename + token + "_"
					elif idx == len(normalization_parameters_filename_tokens)-2:
						new_normalization_parameters_filename = new_normalization_parameters_filename + token
				new_normalization_parameters_filename = new_normalization_parameters_filename + "." + normalization_parameters_filename.split(".")[-1]		
					
				os.rename(input_dir + run_mode + "/AD/Inference/Normalization/" + normalization_parameters_filename, input_dir + run_mode + "/AD/Inference/Normalization/" + new_normalization_parameters_filename)
				
		for compression_parameters_filename in os.listdir(input_dir + run_mode + "/AD/Inference/Compression"):
			if compression_parameters_filename.split(".")[0].split("_")[-1] != str(best_accuracy_model):
				os.remove(input_dir + run_mode + "/AD/Inference/Compression/" + compression_parameters_filename)
			else:
				compression_parameters_filename_tokens = compression_parameters_filename.split(".")[0].split("_")
				new_compression_parameters_filename = ""
				for idx,token in enumerate(compression_parameters_filename_tokens):
					if idx < len(compression_parameters_filename_tokens)-2:
						new_compression_parameters_filename = new_compression_parameters_filename + token + "_"
					elif idx == len(compression_parameters_filename_tokens)-2:
						new_compression_parameters_filename = new_compression_parameters_filename + token
				new_compression_parameters_filename = new_compression_parameters_filename + "." + compression_parameters_filename.split(".")[-1]		
					
				os.rename(input_dir + run_mode + "/AD/Inference/Compression/" + compression_parameters_filename, input_dir + run_mode + "/AD/Inference/Compression/" + new_compression_parameters_filename)		
		...
		if keep_sa_model == True:
			...
			for normalization_parameters_filename in os.listdir(input_dir + run_mode + "/AD/Inference/Normalization"):
				if normalization_parameters_filename.split(".")[0].split("_")[-1] != "SA":
					os.remove(input_dir + run_mode + "/AD/Inference/Normalization/" + normalization_parameters_filename)
				else:
					normalization_parameters_filename_tokens = normalization_parameters_filename.split(".")[0].split("_")
					new_normalization_parameters_filename = ""
			...
			for compression_parameters_filename in os.listdir(input_dir + run_mode + "/AD/Inference/Compression"):
				if compression_parameters_filename.split(".")[0].split("_")[-1] != "SA":
					os.remove(input_dir + run_mode + "/AD/Inference/Compression/" + compression_parameters_filename)
				else:
					compression_parameters_filename_tokens = compression_parameters_filename.split(".")[0].split("_")
					new_compression_parameters_filename = ""
			...
			for normalization_parameters_filename in os.listdir(input_dir + run_mode + "/AD/Inference/Normalization"):
				if normalization_parameters_filename.split(".")[0].split("_")[-1] != str(best_accuracy_model) and normalization_parameters_filename.split(".")[0].split("_")[-1] != "temp":
					os.remove(input_dir + run_mode + "/AD/Inference/Normalization/" + normalization_parameters_filename)
				elif normalization_parameters_filename.split(".")[0].split("_")[-1] != "temp":
					normalization_parameters_filename_tokens = normalization_parameters_filename.split(".")[0].split("_")
					new_normalization_parameters_filename = ""
					for idx,token in enumerate(normalization_parameters_filename_tokens):
						if idx < len(normalization_parameters_filename_tokens)-2:
			...
			for compression_parameters_filename in os.listdir(input_dir + run_mode + "/AD/Inference/Compression"):
				if compression_parameters_filename.split(".")[0].split("_")[-1] != str(best_accuracy_model) and compression_parameters_filename.split(".")[0].split("_")[-1] != "temp":
					os.remove(input_dir + run_mode + "/AD/Inference/Compression/" + compression_parameters_filename)
				elif compression_parameters_filename.split(".")[0].split("_")[-1] != "temp":
					compression_parameters_filename_tokens = compression_parameters_filename.split(".")[0].split("_")
					new_compression_parameters_filename = ""
					for idx,token in enumerate(compression_parameters_filename_tokens):
						if idx < len(compression_parameters_filename_tokens)-2:

...
for /l %%x in (1, 1, %nreps%) do (
	...
	python anomaly_detection.py FirstRun training %debug_mode% %sample_time% %n% %use_categorical_labels% %validation_split_percentage% %hidden_neurons% %latent_code_dimension% %epochs%

	copy Output\FirstRun\AD\Training\Model\* Input\FirstRun\AD\Inference\Model

	python anomaly_detection.py FirstRun inference %debug_mode% %sample_time% %n% %use_categorical_labels%
	...
	ren Output\FirstRun\AD\Training\Model\n_grams_list.txt n_grams_list_%%x.txt
